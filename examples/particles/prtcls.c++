...
{
  if (mode == 0) {
      return Tile::send_particle_data(comm, dest, tag);
  } else if(mode == 1) {
      return Tile::send_particle_extra_data(comm,dest, tag);
  } else {
      assert(false);
      exit(1);
  }
...
{
  if(mode == 0) {
      return Tile::recv_particle_data(comm,orig,tag);
  } else if(mode == 1) {
      return Tile::recv_particle_extra_data(comm,orig,tag);
  } else { 
      assert(false);
      exit(1);
  }
