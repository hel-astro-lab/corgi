...
def plotNode(ax, n, conf, mpigrid=False):
    ...
    tmp_grid = np.ones( (n.get_Nx(), n.get_Ny(), n.get_Nz() ) ) * -1.0
    for cid in n.get_tile_ids():
        c = n.get_tile( cid )
        (i, j, k) = c.index

        #check dublicates
        if tmp_grid[i,j,k] != -1.0:
            print("{}: ERROR in real cells at ({},{},{})".format(n.rank(), i,j,k))
            sys.exit()
        tmp_grid[i,j,k] = c.communication.owner

    if not(mpigrid):
        imshow(ax, tmp_grid[:,:,kslice], 
    ...
    # internal mpi_grid
    if mpigrid:
        tmp_grid2 = np.ones( (n.get_Nx(), n.get_Ny(), n.get_Nz()) ) * -1.0
        for i in range(n.get_Nx()):
            for j in range(n.get_Ny()):
                for k in range(n.get_Nz()):
                    tmp_grid2[i,j,k] = grid.get_mpi_grid(i,j,k)
...
def get_mpi_grid(n, conf):
    ...
    tmp = np.ones( (n.get_Nx(), n.get_Ny(), n.get_Nz()) ) * -1.0
    for i in range(n.get_Nx()):
        for j in range(n.get_Ny()):
            for k in range(n.get_Nz()):
                tmp[i,j,k] = grid.get_mpi_grid(i,j,k)
    return tmp
...
def get_work_grid(n, conf):
    ...
    for i in range(n.get_Nx()):
        for j in range(n.get_Ny()):
            for k in range(n.get_Nz()):
                tmp[i,j,k] = grid.get_work_grid(i,j,k)
    return tmp
...
def plotWork(ax, n, conf):
    ...
    for i in range(n.get_Nx()):
        for j in range(n.get_Ny()):
            for k in range(n.get_Nz()):
                tmp_grid[i,j,k] = grid.get_work_grid(i,j,k)
    ...
    for i in range(grid.get_Nx()):
        for j in range(grid.get_Ny()):
            for k in range(grid.get_Nz()):
                if grid.get_mpi_grid(i,j,k) == grid.rank():
                        ...
                        for n in range(conf.NzMesh):
                            for m in range(conf.NyMesh):
                                for l in range(conf.NxMesh):
                                    ...
                                    xloc = spatialLoc(grid, (i,j,k), (l,m,n), conf)
...
if __name__ == "__main__":
    ...
    f5.create_dataset("grid", (conf.Nx, conf.Ny, conf.Nz, Nsamples), dtype='f')
    f5.create_dataset("work", (conf.Nx, conf.Ny, conf.Nz, Nsamples), dtype='f')
